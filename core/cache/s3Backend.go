...
func NewS3Backend(s3Config *aws.Config, keyPrefix string, bucketName string, frontendName string) *S3Backend {
	...
	return &S3Backend{
		cacheMetrics: NewCacheMetrics("s3", frontendName),
		keyPrefix:    keyPrefix,
...
// Get entry by tag
func (b *S3Backend) Get(key string) (entry *Entry, found bool) {
...
// Set an cache key
func (b *S3Backend) Set(key string, entry *Entry) error {
...
func (b *S3Backend) buildEntry(entry *Entry) *S3CacheEntry {
	return &S3CacheEntry{
		Meta: s3CacheEntryMeta{
...
func (b *S3Backend) buildResult(entry *S3CacheEntry) *Entry {
	return &Entry{
		Meta: Meta{
			Lifetime:  entry.Meta.Lifetime,
			Gracetime: entry.Meta.Gracetime,
